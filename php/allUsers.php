...
<body>
    ...
    <th>Ext</th><th>Image</th><th>Role</th><th colspan=2>Action</th></tr>';
    foreach ($result as $user) {
        ...
        </td><td class='align-middle'>". $user['role']."</td><td class='align-middle'><a href=delete.php/?username=".$user['username'].">
        <button class='btn btn-danger'>delete</button></a></td><td class='align-middle'><a href=editUser.php/?username=".$user['username']."&email=".$user['email']."&room=".$user['room']."&ext=".$user['ext']."&profile_pic=".$user['profile_pic']."&role=".$user['role'].">
        <button class='btn btn-primary'>update</button></a></td></tr>" ;
    }
    echo '</table>';
?>
</body>
